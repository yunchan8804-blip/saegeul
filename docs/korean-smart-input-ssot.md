...
이 문서는 `D:\workspace\Saegul`에서 진행하는 한국어 특화 입력, 생성형 AI,
...
최종 갱신일: 2026-09-09
기준 브랜치: `YunChan/gemma-ondevice-materials`
현재 활성 구현 마일스톤: `IN_PROGRESS — 온디바이스 재료 생성·영구 저장·로컬 추천 실험`

### 온디바이스 재료 생성 계약

AI가 미리 만든 재료를 검증·저장하고 입력할 때 로컬에서 조회·조합하는 것이 목표다. 동일 입력 문맥에 대한 일시적인 LLM 응답 캐시는 영구 재료 저장소를 대신하지 않는다. 생성 재료는 사용자가 실제 작성한 문장과 출처를 구분하며, 공개 문장팩과 개인 금고도 분리한다.

현재 Gemma 실험은 디버그 앱의 명시적 다운로드·생성에 한정한다. 고정된 비개인 주제로 생성하고, 입력 중 자동 실시간 AI 요청을 끈 상태에서 저장 재료의 재사용을 검증한다. 기존 정식 앱의 공급자 계약은 이 실험만으로 변경하지 않는다. 모델 없이도 기본 로컬 추천은 동작해야 한다. 상세 경계·단계·완료 증거는 [실험 설계](gemma-ondevice-experiment-2026-09-09.md)를 따른다.
